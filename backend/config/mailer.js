...
const sendCreateAlerEmail = (toEmail, name, employeeId) => {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: toEmail,
...
const sendWelcomeEmail = (toEmail, name, employeeId) => {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: toEmail,
    subject: "Welcome to Trade Syndicate",
    text: `Dear ${name},\n\nYour Trade Syndicate account has been created successfully.\nYour employeeId is: ${employeeId}\n Thank you for joining the Trade Syndicate family!\n\nBest regards,\nThe Trade Syndicate Team`,
  };
  return transporter.sendMail(mailOptions);
};
