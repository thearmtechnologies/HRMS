...
const ANALYTICS = {
  headcount: [
    { label: "IT", value: 45, color: "bg-[#3d766d]" },
    { label: "Operations", value: 22, color: "bg-[#3d766d]/80" },
    { label: "Finance", value: 18, color: "bg-[#3d766d]/60" },
    { label: "Marketing", value: 14, color: "bg-[#3d766d]/40" },
    { label: "HR", value: 12, color: "bg-[#3d766d]/20" },
  ],
  budget: [
...
const Badge = ({ children, type = "default" }) => {
  const styles = {
    success: "bg-[#3d766d]/10 text-[#3d766d]",
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 lg:space-y-8 pb-12 font-sans text-[#8f9192]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#3d766d]">Departments</h1>
            ...
            <Card key={dept.id} className="flex flex-col group">
              <div className="p-5 border-b border-[#d6d9df]">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-[#3d766d] leading-tight">{dept.name}</h3>
                ...
                <div className="space-y-3 mt-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#f0f3f5] text-[#3d766d] flex items-center justify-center font-bold text-xs shrink-0">
                      {dept.head.charAt(0)}
                    </div>
                    <div>
                      <p className="text-xs text-[#bdc2c7]">Department Head</p>
                      <p className="text-sm font-semibold text-[#3d766d]">{dept.head}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-[#f0f3f5]/50 flex items-center justify-between mt-auto">
                <div className="flex gap-4">
                  <div className="text-center">
                    <p className="text-lg font-bold text-[#3d766d]">{dept.employees}</p>
                    <p className="text-xs text-[#8f9192]">Employees</p>
                  </div>
                  <div className="w-px bg-[#d6d9df]"></div>
                  <div className="text-center">
                    <p className="text-lg font-bold text-[#3d766d]">{dept.projects}</p>
                    <p className="text-xs text-[#8f9192]">Projects</p>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedDept(dept)}
                  className="px-4 py-2 bg-[#fdfdfe] border border-[#d6d9df] text-[#3d766d] rounded-lg text-sm font-semibold hover:border-[#3d766d] transition-colors shadow-sm"
                >
                  View Details
                </button>
              </div>
              ...
              {DEPARTMENTS.map((dept) => (
                <tr key={dept.id} className="hover:bg-[#f0f3f5]/50 transition-colors">
                  <td className="px-5 py-4">
                    <p className="font-bold text-[#3d766d]">{dept.name}</p>
                    <p className="text-xs text-[#bdc2c7]">{dept.id}</p>
                  </td>
                  <td className="px-5 py-4 font-medium text-[#3d766d]">{dept.head}</td>
              ...
              <div className="space-y-3">
                {ANALYTICS.budget.map((item, i) => (
                  <div key={i} className="flex items-center text-sm">
                    ...
                    <span className="w-10 text-right font-bold text-[#3d766d]">{item.display}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="relative border-l-2 border-[#d6d9df] ml-3 space-y-6">
            {ACTIVITIES.map((item, idx) => (
              <div key={idx} className="relative pl-6">
                <div className="absolute w-3 h-3 bg-[#3d766d] rounded-full -left-[7px] top-1.5 shadow-[0_0_0_4px_#fdfdfe]"></div>
      ...
      {selectedDept && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          {/* Backdrop */}
          <div 
            className="absolute inset-0 bg-[#3d766d]/20 backdrop-blur-sm"
            ...
            <div className="shrink-0 p-5 sm:p-6 border-b border-[#d6d9df] flex items-start justify-between bg-[#fdfdfe]">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h2 className="text-2xl font-bold text-[#3d766d]">{selectedDept.name}</h2>
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  onClick={() => setSelectedDept(null)}
                  className="p-2 text-[#bdc2c7] hover:text-[#3d766d] hover:bg-[#f0f3f5] rounded-full transition-colors"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <Card className="p-5">
                      <h3 className="text-sm font-bold text-[#3d766d] uppercase tracking-wider mb-4 border-b border-[#d6d9df] pb-2">Leadership</h3>
                      <div className="space-y-3 text-sm">
                        <div className="flex justify-between"><span className="text-[#bdc2c7]">Dept Head</span> <span className="font-bold text-[#3d766d]">{selectedDept.leadership.head}</span></div>
                  ...
                  {/* Workforce & Hierarchy */}
                  <Card className="p-5">
                    <h3 className="text-sm font-bold text-[#3d766d] uppercase tracking-wider mb-4 border-b border-[#d6d9df] pb-2 flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      <div>
                        <div className="space-y-3 text-sm">
                          ...
                          <div className="flex justify-between items-center"><span className="text-[#bdc2c7]">New Hires (YTD)</span> <span className="font-semibold text-[#3d766d] bg-[#3d766d]/10 px-2 py-0.5 rounded">+{selectedDept.workforce.newHires}</span></div>
                        </div>
                      </div>

                      {/* Hierarchy Tree Visual */}
                      <div className="bg-[#fdfdfe] border border-[#d6d9df] rounded-lg p-4 text-sm font-medium">
                        <div className="flex items-center gap-2 text-[#3d766d] font-bold">
                  ...
                  <Card className="p-0 overflow-hidden">
                    <div className="p-5 border-b border-[#d6d9df]">
                      <h3 className="text-sm font-bold text-[#3d766d] uppercase tracking-wider">Current Projects</h3>
                    </div>
                        ...
                        <tbody className="divide-y divide-[#d6d9df]">
                          {selectedDept.activeProjects.map((proj, i) => (
                            <tr key={i} className="hover:bg-[#f0f3f5]/50">
                              <td className="px-5 py-3 font-bold text-[#3d766d]">{proj.name}</td>
                ...
                <div className="space-y-6">
                  ...
                  <Card className="p-5">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-xl font-bold text-[#3d766d]">{formatCurrency(selectedDept.budget.annual)}</p>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between text-xs font-bold mb-1">
                          ...
                          <span className="text-[#3d766d]">{Math.round((selectedDept.budget.used / selectedDept.budget.annual) * 100)}%</span>
                        </div>
                        <div className="w-full h-2 bg-[#f0f3f5] rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-[#3d766d] rounded-full" 
                  ...
                  {/* KPIs */}
                  <Card className="p-5 bg-[#3d766d] text-[#fdfdfe] border-none relative overflow-hidden">
                  ...
                  <Card className="p-5">
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-bold text-[#3d766d]">{selectedDept.assets.laptops}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-2 text-[#8f9192]"><Monitor size={14}/> Monitors</span>
                        <span className="font-bold text-[#3d766d]">{selectedDept.assets.monitors}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-2 text-[#8f9192]"><Network size={14}/> Servers</span>
                        <span className="font-bold text-[#3d766d]">{selectedDept.assets.servers}</span>
                      </div>
                    </div>
                  </Card>
